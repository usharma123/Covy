...
| Check runbook density | `node scripts/check_context_anomaly_runbook_density.mjs --self-test` | Prints one success line; `--json` emits `line_count`, `max_lines`, `commands_checked`, and `max_json_bytes`; env knobs are `P28_CONTEXT_ANOMALY_RUNBOOK_MAX_LINES` and `P28_CONTEXT_ANOMALY_RUNBOOK_JSON_MAX`; `--help` lists modes; failures are `context_anomaly_runbook_density_too_many_lines` or `context_anomaly_runbook_density_missing_commands`. |
