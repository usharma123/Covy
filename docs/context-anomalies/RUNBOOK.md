...
| Check runbook density | `node scripts/check_context_anomaly_runbook_density.mjs --self-test` | Prints one success line; `--json` emits `line_count`, `max_lines`, and `commands_checked`; env knob is `P28_CONTEXT_ANOMALY_RUNBOOK_MAX_LINES`; failures are `context_anomaly_runbook_density_too_many_lines` or `context_anomaly_runbook_density_missing_commands`. |
