...
| Read runbook density | `node scripts/check_context_anomaly_runbook_density.mjs --json` | JSON emits budgets, counts, and `max_json_bytes`; default text is one-line `key=value`; labels: `failure_codes`, `env_docs`, `output_labels`, `workflow_commands`, `prose`, `json_headroom`; envs below. |
