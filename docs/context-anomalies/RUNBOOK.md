...
| Read runbook density | `node scripts/check_context_anomaly_runbook_density.mjs --json` | JSON emits budgets/counts; default text is one-line `key=value`; labels: `failure_codes`, `env_docs`, `output_labels`, `output_doc_phrases`, `workflow_commands`, `prose`, `json_headroom`, `text_width_docs`, `text_width`; `text_width` is capped by `P28_CONTEXT_ANOMALY_RUNBOOK_TEXT_MAX`; envs below. |
