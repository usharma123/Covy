...
| Read runbook density | `node scripts/check_context_anomaly_runbook_density.mjs --json` | JSON keeps full field names; incl. `alias_docs_checked`; `max_json_bytes=448`; `key=value`: `lines`, `row`, `cmds`, `fc`, `env`, `labels`, `phrases`, `adocs`, `dphr`, `anc`, `soft`, `parsed`, `wf`, `prose`, `json`, `wdocs`, `width`; m: `fc`=failure codes, `wf`=workflow commands, `json`=remaining JSON headroom; `width` is capped by `P28_CONTEXT_ANOMALY_RUNBOOK_TEXT_MAX`; env. |
