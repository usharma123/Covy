...
| Read runbook density | `node scripts/check_context_anomaly_runbook_density.mjs --json` | JSON keeps full field names incl. `alias_docs_checked`; budget `max_json_bytes=352`; text `key=value` aliases: `lines`, `row`, `cmds`, `fc`, `env`, `labels`, `phrases`, `alias_docs`, `soft`, `wf`, `prose`, `json`, `width_docs`, `width`; map: `fc`=failure codes, `wf`=workflow commands, `json`=remaining JSON headroom; `width` is capped by `P28_CONTEXT_ANOMALY_RUNBOOK_TEXT_MAX`; envs. |
| Inspect digest | `Packet28 digest --root . --json` | Shows visible anomalies and capped `hidden_samples`. |
Density envs: `P28_CONTEXT_ANOMALY_RUNBOOK_MAX_LINES`, `P28_CONTEXT_ANOMALY_RUNBOOK_ROW_MAX`, `P28_CONTEXT_ANOMALY_RUNBOOK_ROW_SOFT_MAX`, `P28_CONTEXT_ANOMALY_RUNBOOK_PROSE_MAX`, `P28_CONTEXT_ANOMALY_RUNBOOK_TEXT_MAX`, `P28_CONTEXT_ANOMALY_RUNBOOK_JSON_MAX`, `P28_CONTEXT_ANOMALY_RUNBOOK_JSON_HEADROOM_MIN`; JSON soft fields: `row_soft_ok`, `row_soft_max`; help budget: `help<=120`.
