...
| Read runbook density | `node scripts/check_context_anomaly_runbook_density.mjs --json` | JSON emits budgets, counts, and `max_json_bytes`; default text is one-line `key=value`; labels: `failure_codes`, `env_docs`, `output_labels`, `output_doc_phrases`, `workflow_commands`, `prose`, `json_headroom`; envs below. |
| Inspect digest | `Packet28 digest --root . --json` | Shows visible anomalies and capped `hidden_samples`. |
Density envs: `P28_CONTEXT_ANOMALY_RUNBOOK_MAX_LINES`, `P28_CONTEXT_ANOMALY_RUNBOOK_ROW_MAX`, `P28_CONTEXT_ANOMALY_RUNBOOK_PROSE_MAX`, `P28_CONTEXT_ANOMALY_RUNBOOK_JSON_MAX`, `P28_CONTEXT_ANOMALY_RUNBOOK_JSON_HEADROOM_MIN`.
Density failures: `context_anomaly_runbook_density_too_many_lines`, `context_anomaly_runbook_density_row_too_wide`, `context_anomaly_runbook_density_missing_commands`, `context_anomaly_runbook_density_workflow_missing_commands`, `context_anomaly_runbook_density_missing_env_docs`.
