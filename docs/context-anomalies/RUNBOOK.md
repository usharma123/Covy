...
| Read runbook density | `node scripts/check_context_anomaly_runbook_density.mjs --json` | Emits line/table/prose budgets such as `max_density_prose_line`, count fields, and `max_json_bytes`; text labels include `failure_codes`, `workflow_commands`, and `prose`; env knobs are `P28_CONTEXT_ANOMALY_RUNBOOK_MAX_LINES`, `P28_CONTEXT_ANOMALY_RUNBOOK_ROW_MAX`, `P28_CONTEXT_ANOMALY_RUNBOOK_PROSE_MAX`, and `P28_CONTEXT_ANOMALY_RUNBOOK_JSON_MAX`. |
