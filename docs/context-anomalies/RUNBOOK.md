...
| Read runbook density | `node scripts/check_context_anomaly_runbook_density.mjs --json` | Emits line/table/prose budgets, counts, and `max_json_bytes`; labels: `failure_codes`, `env_docs`, `workflow_commands`, `prose`, `json_headroom`; envs below. |
