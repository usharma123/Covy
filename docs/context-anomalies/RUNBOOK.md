...
| Read runbook density | `node scripts/check_context_anomaly_runbook_density.mjs --json` | JSON keeps full field names; `max_json_bytes=576`; `key=value`: `lines`, `row`, `cmds`, `fc`, `env`, `labels`, `phrases`, `adocs`, `dphr`, `anc`, `soft`, `parsed`, `wf`, `prose`, `dlab`, `jhead`, `jpar`, `wdocs`, `width`; m: `fc`=failure codes, `wf`=workflow commands, `jhead`=JSON headroom, `dlab`=density label width; `width` is capped by `P28_CONTEXT_ANOMALY_RUNBOOK_TEXT_MAX`; env. |
...
JSON:`alias_docs_checked`,`row_soft_ok`,`row_soft_max`,`output_doc_phrases_checked`,`density_doc_phrases_checked`,`density_doc_anchors_checked`,`parsed_fields_checked`,`json_parity_fields_checked`,`density_label_line_width`,`text_width_docs_checked`;h:`help<=120`.
Density labels:`Env:`=env,`JSON:`=fields,`h:`=help,`jpar`=JSON parity,`adocs`=alias docs,`wdocs`=width docs,`jhead` uses `P28_CONTEXT_ANOMALY_RUNBOOK_JSON_HEADROOM_MIN`.
