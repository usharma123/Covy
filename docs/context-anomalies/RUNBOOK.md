...
| Read runbook density | `node scripts/check_context_anomaly_runbook_density.mjs --json` | JSON keeps full field names; default text is one-line `key=value` aliases: `lines`, `row`, `cmds`, `fc`, `env`, `labels`, `phrases`, `wf`, `prose`, `json`, `width_docs`, `width`; alias map: `fc`=failure codes, `wf`=workflow commands, `json`=JSON headroom; `width` is capped by `P28_CONTEXT_ANOMALY_RUNBOOK_TEXT_MAX`; envs below. |
