...
| Read runbook density | `node scripts/check_context_anomaly_runbook_density.mjs --json` | Emits `line_count`, `max_lines`, `max_table_row`, `commands_checked`, `failure_codes_checked`, `workflow_commands_checked`, and `max_json_bytes`; text labels include `failure_codes` and `workflow_commands`; env knobs are `P28_CONTEXT_ANOMALY_RUNBOOK_MAX_LINES`, `P28_CONTEXT_ANOMALY_RUNBOOK_ROW_MAX`, and `P28_CONTEXT_ANOMALY_RUNBOOK_JSON_MAX`. |
| Inspect digest | `Packet28 digest --root . --json` | Shows visible anomalies and capped `hidden_samples`. |

Density failures: `context_anomaly_runbook_density_too_many_lines`, `context_anomaly_runbook_density_row_too_wide`, `context_anomaly_runbook_density_missing_commands`, `context_anomaly_runbook_density_workflow_missing_commands`, `context_anomaly_runbook_density_missing_failure_docs`, `context_anomaly_runbook_density_missing_output_docs`, `context_anomaly_runbook_density_json_too_long`. `Packet28 dashboard --root . --json` reads live history and reports latest status, high count, hidden categories, and recurring hidden categories.
