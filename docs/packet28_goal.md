...
- Packet28 evidence confidence now labels backing directly as `backing=artifact` or `backing=missing`, making artifact-backed versus unbacked symbol evidence readable without interpreting artifact-gap counts. Evidence: `broker_evidence_confidence_orders_symbol_evidence_tiers`.
