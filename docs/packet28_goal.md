...
- Packet28 broker plan validation now uses cached test-map coverage when validating uncovered edit steps: a later test gate must target the mapped tests for the edited path when Packet28 has that mapping, while generic full-suite test steps remain valid. This makes plan validation more useful for agentic coding handoffs by catching irrelevant test gates before implementation starts. Evidence: `validate_plan_requires_testmap_mapped_gate_for_uncovered_edits`; `validate_plan_accepts_testmap_mapped_or_generic_test_gate`.
