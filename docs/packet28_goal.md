...
- RTK `gain` coverage now includes direct RTK-style mode flags in addition to Packet28's `--format` selector: `--graph`, `--history`, `--failures`, `--daily`, `--weekly`, `--monthly`, `--quota`, and `--all`; quota mode accepts RTK-style `--tier pro|5x|20x` plus Packet28's explicit `--quota-tokens` override. Evidence: `test_run_reduces_git_status`; `test_gain_reports_failed_and_fallback_runs`.
