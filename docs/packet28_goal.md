...
- RTK system-command coverage now includes top-level `Packet28 json`, `Packet28 deps`, `Packet28 env`, and `Packet28 log`: JSON compaction/schema rendering with non-JSON extension rejection, dependency manifest summaries for Cargo/package.json/requirements.txt/pyproject.toml/go.mod, categorized environment output with secret masking by default, and log deduplication that normalizes timestamps, IDs, large numbers, hex values, and paths. Evidence: `cmd_system::tests::*`, `test_system_json_deps_and_env_commands`, and `test_system_log_command_deduplicates_noisy_lines`.
