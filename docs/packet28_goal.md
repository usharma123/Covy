...
- `Packet28 gain --failures` now also reports the next successful command in the same working directory after each failure/fallback record, giving repeated failure fingerprints a concrete likely-fix command without replaying full logs. Evidence: updated `test_gain_reports_failed_and_fallback_runs`.
...
- Packet28 broker edit/inspect context now includes `evidence_freshness` when paths or symbols changed since checkpoint, marking which changed paths already have fresh reads and which should be refreshed before relying on cached evidence. Evidence: `broker_edit_context_surfaces_evidence_freshness_for_changed_paths`.
