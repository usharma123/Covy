...
- RTK system/infra wrapper coverage now includes top-level `Packet28 ls`, `Packet28 wc`, `Packet28 wget`, `Packet28 curl`, `Packet28 docker`, and `Packet28 kubectl`, using Packet28's reducer API for compact directory listings, counts, download summaries, HTTP title/byte summaries, container lists, and Kubernetes pending-row summaries. Evidence: `test_system_infra_and_count_commands_use_reducer_wrappers`.
