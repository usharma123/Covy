...
- `Packet28 verify experiments` now validates JSON experiment manifests for covered workflows, command evidence, existing artifact paths, duplicate IDs, and unexpected fallback reasons, making optimization claims auditable against concrete files. Evidence: `test_verify_experiments_checks_manifest_evidence`.
