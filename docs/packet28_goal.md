...
- Packet28 gain JSON now exposes `route_roi`, a per-route ledger of invocation count, compact/raw token totals, saved tokens, savings percent, and average saved tokens per invocation so agents can prefer historically high-value context routes. Evidence: `test_run_reduces_git_status`.
