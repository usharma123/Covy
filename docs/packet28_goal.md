...
- Failed `Packet28 run` records now carry a stable `failure_fingerprint`, and `Packet28 gain --failures` includes the fingerprint column so repeated failures can be grouped for future local advice. Evidence: `test_gain_reports_failed_and_fallback_runs`.
