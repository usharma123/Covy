...
- RTK `gain` coverage now includes direct RTK-style mode flags in addition to Packet28's `--format` selector: `--graph`, `--history`, `--failures`, `--daily`, `--weekly`, `--monthly`, `--quota`, and `--all`. Evidence: `test_run_reduces_git_status`; `test_gain_reports_failed_and_fallback_runs`.
