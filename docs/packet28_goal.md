...
- RTK CLI command-family coverage now includes top-level `Packet28 smart <file>` with local two-line heuristic source summaries and JSON output. Evidence: `smart_summarizes_source_file_with_local_heuristics`; `test_system_smart_command_summarizes_source_file`.
