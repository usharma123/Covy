...
- RTK system-command coverage now includes top-level `Packet28 json`, `Packet28 deps`, and `Packet28 env`: JSON compaction/schema rendering with non-JSON extension rejection, dependency manifest summaries for Cargo/package.json/requirements.txt/pyproject.toml/go.mod, and categorized environment output with secret masking by default. Evidence: `cmd_system::tests::*` and `test_system_json_deps_and_env_commands`.
