...
| FTS search | `memories_fts`, `feedback_fts`, `concepts_fts` | ICM creates FTS5 virtual tables and triggers. | Packet28 now creates `memories_fts`, `feedback_fts_all`, and `concepts_fts` with triggers, uses FTS5 query first, and falls back to LIKE. | partial | Vector hybrid recall remains missing. Current evidence: `test_memory_store_recall_uses_sqlite_home_db`, `test_feedback_and_graph_cli_use_sqlite`, `test_mcp_memory_store_recall_uses_sqlite_home_db`. |
