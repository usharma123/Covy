...
|---|---|---|---|---|---|---|
| COR-01 | Linux `open`/`openat` preload hooks use a fixed ABI for variadic libc symbols. | Confirmed: four fixed-arity Rust exports and four fixed function-pointer casts cross the variadic libc ABI; no ABI test exists. | Preserve the C variadic ABI and only consume `mode_t` for `O_CREAT` or the full `O_TMPFILE` mask. Add public safety contracts around unsafe callbacks. | Linux compile/link ABI regression plus flag/mode/errno behavior tests; focused unsafe Clippy. | Pending | IN PROGRESS |
...
| REP-05 | Workspace lint inheritance is absent; opt-in panic/unsafe/clone/perf lints are not policy. | Confirmed at baseline: no workspace lint table and member inheritance was absent. | Added conservative workspace lint policy and explicit inheritance in all 28 members; strict workspace cleanup remains in progress. | Policy verifier asserts all-member inheritance; strict Clippy surfaced one pre-existing search warning and five suite-cli warnings now tracked for repair. | `018b74a` plus strict-lint follow-up | IN PROGRESS |
...
| ID | Audit finding or recommendation | Current-source validation | Implementation / invariant | Test or mechanical evidence | Closing commit | Status |
|---|---|---|---|---|---|---|
| ARC-01 | Daemon wire DTO/framing depends on kernel, memory, and reducer implementations. | Confirmed: daemon core directly depends on kernel, memory, and reducer implementations; four adapter/CLI consumers import its mixed root surface. | Create a deep protocol crate/module containing only wire DTOs and framing; runtime implementation stays in `packet28d`/runtime ownership. | Cargo metadata negative-dependency assertions; protocol round-trip and compatibility tests for all adapters. | Pending | IN PROGRESS |
...
| ARC-06 | Runtime integration behavior leaks across repeated `RuntimeKind` switchboards. | Confirmed: paths, detection, capabilities, setup, and status are repeated across switchboards for 12 runtimes. | Let each runtime integration own paths, detection, capability, setup, and status while setup orchestrates cross-runtime concerns. | Table-driven integration fixture tests and switchboard/dependency invariant. | Pending | IN PROGRESS |
...
| ID | Audit finding or recommendation | Current-source validation | Implementation / invariant | Test or mechanical evidence | Closing commit | Status |
|---|---|---|---|---|---|---|
| API-01 | Reusable search, daemon, diff, and test libraries leak `anyhow` and erase typed sources into strings. | Confirmed: public signatures include 7 search-core, 22 daemon-core, 5 diffy-core, and multiple test-library `anyhow` results/aliases. | Define stable `thiserror` error seams in libraries; reserve contextual `anyhow` reporting for binaries. | Error variant/source/display tests and public API compile checks. | Pending | IN PROGRESS |
...
| TST-05 | No snapshot coverage exists for structural MCP/protocol output. | Partially reproducible: 39 JSON packet snapshots already exist, but tests assert only two discriminator fields; MCP/protocol exact structural drift is uncovered. | Strengthen the existing JSON golden mechanism and add small reviewed MCP/protocol structural snapshots; do not add a second framework without leverage. | Exact normalized JSON comparison with committed reviewed fixtures. | Pending | PENDING |
...
| PER-05 | Test planning materializes a dense tests×files matrix and clones/rebuilds bitmaps. | Confirmed: planning constructs dense test/file membership work and clones remaining bitmaps in gain calculation. | Persist sparse bitmap rows, intersect by reference, and avoid cloning remaining sets in gain calculation. | Property parity and realistic scale benchmark over tests/files/changed lines. | Pending | IN PROGRESS |
| PER-06 | Map and regex indexes independently walk/read; diagnostic ingestion copies whole files; LCOV allocates per record. | Confirmed for duplicate walks, diagnostics whole-file copies, and LCOV per-record collection; adoption remains measurement-gated. | Share bounded scan/content work and stream/remove copies only where current release benchmarks show leverage. | Allocation/throughput benchmarks plus parser parity/property tests. | Pending | IN PROGRESS |
