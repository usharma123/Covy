...
| Scoped wake-up packs | Reduces startup context by matching memory to the requested path/symbol/task type instead of broad project recall. | Add `--path`, `--symbol`, and `--intent` filters to `Packet28 wakeup` and MCP wakeup. | Wake-up tests prove irrelevant topic memories are pruned under tight budgets. |
