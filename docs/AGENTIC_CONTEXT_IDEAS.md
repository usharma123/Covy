...
| Context ROI ledger | Lets agents prefer commands and memories that historically saved the most tokens for the current repo. | `Packet28 gain --json` now exposes route-level ROI from recorded compact/raw token deltas, including saved tokens, savings percent, and average saved tokens per invocation. | Evidence: `test_run_reduces_git_status`; future slices should persist downstream follow-up counts and show top saved context sources in the dashboard. |
