...
|---|---|---|---|
| Plan-to-test map validation | Prevents agents from claiming weak verification after edits. | Extend broker `validate_plan` to score test gates by mapped impact coverage, broad-suite fallback, missing mappings, and stale cached testmaps. | Unit tests now cover mapped, broad, missing-mapping, and stale-testmap cases; one real-repo plan validation artifact remains. |
