...
| Failure fingerprint memory | Turns repeated build/test failures into reusable local advice. | Failed `Packet28 run` records now include a deterministic `failure_fingerprint`, and `Packet28 gain --failures` surfaces that key for repeated failure triage. | Evidence: `test_gain_reports_failed_and_fallback_runs`; future slices should count repeats and attach successful fix commands or file edits. |
