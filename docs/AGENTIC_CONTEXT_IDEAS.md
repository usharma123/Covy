...
| Hypothesis checkpoints | Helps agents carry explicit assumptions across handoff without replaying long transcripts. | `Packet28 hypothesis add/confirm/reject/list` records active assumptions as broker decision events with `hypothesis:` IDs, so they flow into broker context and handoff state without replaying full transcripts. | Evidence: `test_hypothesis_cli_tracks_active_assumptions`; future slices should add MCP aliases and evidence links. |
