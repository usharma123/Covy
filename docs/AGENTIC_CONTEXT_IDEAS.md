...
| Agent action critic | Catches risky next actions before execution, not after output reduction. | Broker `choose_tool` now emits an `action_critic` section for missing tool intent, destructive command shapes, and broad unscoped searches. | Evidence: `choose_tool_action_critic_flags_missing_intent_and_risky_commands`; future slices should add hook/MCP surfacing and edit-specific read-before-edit warnings. |
