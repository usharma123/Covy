...
| Failure fingerprint memory | Turns repeated build/test failures into reusable local advice. | Failed `Packet28 run` records now include a deterministic `failure_fingerprint`, and `Packet28 gain --failures` surfaces that key with repeat counts plus the next successful same-directory command as fix evidence. | Evidence: `test_gain_reports_failed_and_fallback_runs`; future slices should attach edited files from hook telemetry. |
...
| Experiment manifest verifier | Prevents maturity claims from drifting away from actual artifacts. | `Packet28 verify experiments --manifest <json>` validates JSON experiment manifests for non-empty IDs, workflows, command evidence, existing artifact paths, duplicate IDs, metric thresholds, runtime versions, and unexpected fallback reasons. | Evidence: `test_verify_experiments_checks_manifest_evidence`; future slices should wire real repo manifests into CI/docs. |
