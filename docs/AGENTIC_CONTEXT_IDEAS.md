...
| Hypothesis checkpoints | Helps agents carry explicit assumptions across handoff without replaying long transcripts. | `Packet28 hypothesis add/confirm/reject/list` and MCP `packet28.hypothesis_add/list/resolve` record active assumptions as broker decision events with `hypothesis:` IDs, including optional related paths/symbols that render in broker context and list output. | Evidence: `test_hypothesis_cli_tracks_active_assumptions`; `test_packet28_mcp_hypothesis_tools_track_active_assumptions`; `active_decisions_render_related_paths_and_symbols`; future slices should link hypotheses to concrete artifact IDs. |
