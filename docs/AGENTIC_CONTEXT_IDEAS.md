...
| Experiment manifest verifier | Prevents maturity claims from drifting away from actual artifacts. | `Packet28 verify experiments --manifest <json>` validates JSON experiment manifests for non-empty IDs, workflows, command evidence, existing artifact paths, duplicate IDs, and unexpected fallback reasons. | Evidence: `test_verify_experiments_checks_manifest_evidence`; future slices should record metric thresholds and runtime versions. |
