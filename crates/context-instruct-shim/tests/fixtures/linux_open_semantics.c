...
static int call_open64(const char *path, int dirfd, int flags) {
  (void)dirfd;
...
static int call_openat64(const char *path, int dirfd, int flags) {
  (void)path;
  return openat64(dirfd, "AGENTS.md", flags);
}
...
static void expect_virtualized_read(const struct open_variant *variant,
  ...
  require(seals >= 0 && (seals & required_seals) == required_seals,
          "virtualized descriptor was not fully sealed");
  errno = 0;
...
static void exercise_real_semantics(const struct open_variant *variant,
  ...
  restore_real_file(path);
  fd = variant->call(path, dirfd, O_PATH);
  if (fd < 0) {
    fail("O_PATH instruction open");
  }
  struct stat path_fd;
  struct stat real_path;
  require(fstat(fd, &path_fd) == 0 && stat(path, &real_path) == 0,
          "stat O_PATH instruction");
  require(path_fd.st_dev == real_path.st_dev && path_fd.st_ino == real_path.st_ino,
          "O_PATH did not reference the real instruction file");
  int path_flags = fcntl(fd, F_GETFL);
  require(path_flags >= 0 && (path_flags & O_PATH) == O_PATH,
          "O_PATH status was not preserved");
  close(fd);
...
int main(int argc, char **argv) {
  require(argc == 2, "usage: linux_open_semantics FIXTURE_ROOT");
  ...
  int regular_fd = direct_open(regular_path, O_RDONLY);
  if (regular_fd < 0) {
  ...
  errno = 0;
  require(openat64(regular_fd, "AGENTS.md", O_RDONLY) == -1 &&
              errno == ENOTDIR,
          "openat64 regular-file dirfd did not preserve ENOTDIR");
  close(regular_fd);
  expect_real_contents(path, ORIGINAL_CONTENT);
  for (size_t index = 0; index < variant_count; ++index) {
