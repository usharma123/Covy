...
pub(crate) struct BuiltinExecutionPolicy;

impl ExecutionPolicy for BuiltinExecutionPolicy {
    fn begin(
        &self,
        target: &str,
        request: &KernelRequest,
    ) -> Result<Box<dyn ExecutionPolicyRun>, KernelError> {
        let policy_guard = load_policy_guard(&request.policy_context)?;
        let mut governance = GovernanceAudit {
            enabled: policy_guard.is_some(),
            config_path: policy_guard
                .as_ref()
                .map(|policy| policy.config_path.clone()),
        ...
        if let Some(policy_guard) = &policy_guard {
            if should_enforce_policy_for_target(target) {
                ...
                let input_audits =
                    audit_packets_against_policy(&policy_guard.config, &request.input_packets)?;
                ensure_policy_audits_pass(target, "input", &input_audits)?;
                governance.input_audits = input_audits;
            }
        }

        Ok(Box::new(BuiltinExecutionPolicyRun {
            target: target.to_string(),
            cache_enabled: cache_enabled_for_request(
                target,
                &request.policy_context,
                &request.reducer_input,
            ),
            cache_input: cache_input_for_request(request, target, policy_guard.as_ref()),
            policy_guard,
            governance,
        }))
    }
...
struct BuiltinExecutionPolicyRun {
    target: String,
    cache_enabled: bool,
    cache_input: Value,
...
impl ExecutionPolicyRun for BuiltinExecutionPolicyRun {
    ...
    fn audit_output(&mut self, packets: &[KernelPacket]) -> Result<(), KernelError> {
        let Some(policy_guard) = &self.policy_guard else {
            return Ok(());
        };
        if !should_enforce_policy_for_target(&self.target) {
            return Ok(());
        }
        let output_audits = audit_packets_against_policy(&policy_guard.config, packets)?;
        ensure_policy_audits_pass(&self.target, "output", &output_audits)?;
        self.governance.output_audits = output_audits;
        Ok(())
    }
